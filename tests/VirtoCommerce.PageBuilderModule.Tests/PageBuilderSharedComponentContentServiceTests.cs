...
public class PageBuilderSharedComponentContentServiceTests
{
    [Fact]
    public void TouchMetadata_MarksOnlyModifiedDateAndCannotOverwriteConcurrentRename()
    {
        ...
        var component = new PageBuilderSharedComponentEntity
        {
            Id = "component",
            StoreId = "store",
        ...
        PageBuilderSharedComponentContentService.TouchMetadata(component);
        context.ChangeTracker.DetectChanges();

        var modifiedProperty = Assert.Single(context.Entry(component).Properties, x => x.IsModified);
        Assert.Equal(nameof(PageBuilderSharedComponentEntity.ModifiedDate), modifiedProperty.Metadata.Name);
