...
public class PageBuilderSharedComponentAggregatePersistenceTests
{
    [Fact]
    public async Task SaveWithContentAsync_PersistsAggregateAndPublishesEventsAfterCommit()
    {
        ...
        var component = await context.Set<PageBuilderSharedComponentEntity>()
            .SingleAsync(TestContext.Current.CancellationToken);
        var content = await context.Set<PageBuilderSharedComponentContentEntity>()
            .SingleAsync(TestContext.Current.CancellationToken);
        var assetReference = await context.Set<PageBuilderSharedComponentAssetReferenceEntity>()
            .SingleAsync(TestContext.Current.CancellationToken);
        ...
        Assert.Equal(
            [
                typeof(PageBuilderSharedComponentChangingEvent),
                typeof(PageBuilderSharedComponentChangedEvent),
                typeof(PageBuilderSharedComponentContentChangedEvent),
            ],
            events.EventTypes);
    }
    ...
    public async Task SaveWithContentAsync_CommitFailureRollsBackMetadataContentAndAssetReferences()
    {
        ...
        await using var context = database.CreateContext();
        Assert.Empty(await context.Set<PageBuilderSharedComponentEntity>()
            .ToListAsync(TestContext.Current.CancellationToken));
        Assert.Empty(await context.Set<PageBuilderSharedComponentContentEntity>()
            .ToListAsync(TestContext.Current.CancellationToken));
        Assert.Empty(await context.Set<PageBuilderSharedComponentAssetReferenceEntity>()
            .ToListAsync(TestContext.Current.CancellationToken));
        Assert.Equal([typeof(PageBuilderSharedComponentChangingEvent)], events.EventTypes);
    }
    ...
    public async Task TrySaveContentAsync_WhenIdentityMatches_ReplacesComponentAssetIndexExactly()
    {
        ...
        Assert.Equal([typeof(PageBuilderSharedComponentContentChangedEvent)], events.EventTypes);
    }
    ...
    public async Task SaveContentAsync_ConcurrentWritersKeepContentAndAssetIndexOnSameVersion()
    {
        ...
        var firstService = new PageBuilderSharedComponentContentService(
    ...
    public async Task SaveWithContentAsync_AndSaveContentAsync_SerializeThroughSameComponentLock()
    {
        ...
        using var cache = new TestPlatformMemoryCache();
        var aggregateService = new PageBuilderSharedComponentService(
            () => new CoordinatedPageBuilderModuleRepository(
                database.CreateContext(),
                async cancellationToken =>
                {
    ...
    public async Task SaveChangesAsync_WhenComponentWasConcurrentlyDeleted_DoesNotResurrectMetadataOnlyRow()
    {
        ...
        {
            var component = await deleteContext.Set<PageBuilderSharedComponentEntity>()
                .SingleAsync(TestContext.Current.CancellationToken);
        ...
        Assert.False(await verifyContext.Set<PageBuilderSharedComponentEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
        Assert.False(await verifyContext.Set<PageBuilderSharedComponentContentEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
        Assert.Empty(events.EventTypes);
    }
    ...
    public async Task UpdateMetadataAsync_WhenIdWasRecreatedInAnotherStore_DoesNotUpdateReplacement()
    {
        ...
        {
            var oldComponent = await replaceContext.Set<PageBuilderSharedComponentEntity>()
                .SingleAsync(TestContext.Current.CancellationToken);
            ...
            replaceContext.Add(new PageBuilderSharedComponentEntity
            {
                Id = ComponentId,
                StoreId = "other-store",
        ...
        Assert.Null(result);
        await using var verifyContext = database.CreateContext();
        var replacement = await verifyContext.Set<PageBuilderSharedComponentEntity>()
            .SingleAsync(TestContext.Current.CancellationToken);
    ...
    public async Task TrySaveContentAsync_WhenAuthorizedIdWasRecreatedInSameStore_DoesNotModifyReplacement()
    {
        ...
        Assert.False(saved);
        await using var context = database.CreateContext();
        var replacement = await context.Set<PageBuilderSharedComponentEntity>()
            .SingleAsync(TestContext.Current.CancellationToken);
        var replacementContent = await context.Set<PageBuilderSharedComponentContentEntity>()
            .Select(x => x.ComponentContent)
            .SingleAsync(TestContext.Current.CancellationToken);
        var assetReferences = await context.Set<PageBuilderSharedComponentAssetReferenceEntity>()
            .Select(x => x.NormalizedAssetUrl)
    ...
    public async Task TryDeleteAsync_WhenIdentityMatches_DeletesCompleteAggregateAndPublishesEvents()
    {
        ...
        Assert.True(deleted);
        await using var context = database.CreateContext();
        Assert.False(await context.Set<PageBuilderSharedComponentEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
        Assert.False(await context.Set<PageBuilderSharedComponentContentEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
        Assert.False(await context.Set<PageBuilderSharedComponentAssetReferenceEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
        Assert.Equal(
            [
                typeof(PageBuilderSharedComponentChangingEvent),
    ...
    public async Task ConditionalOperations_WhenRequiredContentIsMissing_RefuseWithoutRepairingOrDeletingMetadata()
    {
        ...
        Assert.False(deleted);
        await using var context = database.CreateContext();
        Assert.True(await context.Set<PageBuilderSharedComponentEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
        Assert.False(await context.Set<PageBuilderSharedComponentContentEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
    ...
    public async Task SaveWithContentAsync_WhenComponentWasDeleted_DoesNotRecreateIt()
    {
        ...
        Assert.False(await verificationContext.Set<PageBuilderSharedComponentEntity>()
            .AnyAsync(TestContext.Current.CancellationToken));
        Assert.Empty(events.EventTypes);
    }
    ...
    public async Task SaveWithContentAsync_NewExplicitImportId_PersistsAggregate()
    {
        ...
        var component = await context.Set<PageBuilderSharedComponentEntity>()
            .SingleAsync(TestContext.Current.CancellationToken);
        Assert.Equal(model.Id, component.Id);
        Assert.True(await context.Set<PageBuilderSharedComponentContentEntity>()
    ...
    public async Task SaveChangesAsync_WhenSecondComponentWasRecreated_RollsBackWholeBatch()
    {
        ...
        await using var context = database.CreateContext();
        var names = await context.Set<PageBuilderSharedComponentEntity>()
    ...
    public async Task SaveChangesAsync_UpdatesBatchAndPreservesAuditOwnership()
    {
        ...
        await using var context = database.CreateContext();
        var components = await context.Set<PageBuilderSharedComponentEntity>()
            .OrderBy(x => x.Id)
            .ToArrayAsync(TestContext.Current.CancellationToken);
        ...
        Assert.Equal(
            [typeof(PageBuilderSharedComponentChangingEvent), typeof(PageBuilderSharedComponentChangedEvent)],
            events.EventTypes);
    }
    ...
    private static async Task AssertContentAndRecoveryMarkerWereCommittedAsync(TestDatabase database)
    {
        ...
        var componentModifiedDate = await context.Set<PageBuilderSharedComponentEntity>()
            .Where(x => x.Id == ComponentId)
            .Select(x => x.ModifiedDate)
            .SingleAsync(TestContext.Current.CancellationToken);
    ...
    private sealed class ThrowingContentChangedEventPublisher : IEventPublisher
    {
        public Task Publish<T>(T @event, CancellationToken cancellationToken = default)
            where T : IEvent
        {
            if (@event is PageBuilderSharedComponentContentChangedEvent)
            {
    ...
    {
        public override Task<bool> ExecuteUnderSharedComponentWriteLockAsync(
            string sharedComponentId,
            Func<CancellationToken, Task> operation,
            CancellationToken cancellationToken = default)
        {
            return base.ExecuteUnderSharedComponentWriteLockAsync(
                sharedComponentId,
    ...
    private sealed class TestDatabase : IAsyncDisposable
    {
            ...
            DateTime createdDate)
        {
            ...
            context.Add(new PageBuilderSharedComponentAssetReferenceEntity
            {
        ...
        public async Task<PageBuilderSharedComponent> LoadComponentAsync()
        {
            return await LoadComponentAsync(ComponentId);
        }

        public async Task<PageBuilderSharedComponent> LoadComponentAsync(string id)
        {
            await using var context = CreateContext();
            var entity = await context.Set<PageBuilderSharedComponentEntity>()
                .SingleAsync(x => x.Id == id, TestContext.Current.CancellationToken);
            return entity.ToModel(new PageBuilderSharedComponent());
        }
            ...
            string assetUrl)
        {
            ...
            {
                var oldComponent = await deleteContext.Set<PageBuilderSharedComponentEntity>()
                    .SingleAsync(x => x.Id == id, TestContext.Current.CancellationToken);
            ...
            createContext.Add(new PageBuilderSharedComponentEntity
            {
                Id = id,
                StoreId = storeId,
                Name = "Replacement component",
            ...
            createContext.Add(new PageBuilderSharedComponentAssetReferenceEntity
            {
                Id = Guid.NewGuid().ToString("N"),
                SharedComponentId = id,
                NormalizedAssetUrl = assetUrl,
        ...
        public async Task FailSecondComponentUpdatesAsync()
        {
            ...
            await context.Database.ExecuteSqlRawAsync(
                ...
                BEFORE UPDATE ON PageBuilderSharedComponent
        ...
        public async Task FailComponentAssetReferenceInsertsAsync()
        {
            ...
            await context.Database.ExecuteSqlRawAsync(
                ...
                BEFORE INSERT ON PageBuilderSharedComponentAssetReference
                BEGIN
