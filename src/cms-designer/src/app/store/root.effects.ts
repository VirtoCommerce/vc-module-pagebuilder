...
export class RootEffects {
    ...
    @Effect()
    uploadPreviewPresetFalls$ = this.actions$.pipe(
        ofType(themeActions.ThemeActionTypes.UpdateDraftFail),
        withLatestFrom(
            this.rootStore$.select(fromRoot.getSecondaryFrameId),
            this.rootStore$.select(fromRoot.getSecondaryIsLoaded)
        ),
        switchMap(([, frameId, previewReady]) => {
            ...
            return of(new rootActions.PreviewLoading(true, 'update draft falls'));
        })
    );
