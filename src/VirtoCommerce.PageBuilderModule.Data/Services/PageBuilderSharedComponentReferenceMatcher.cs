...
public static class PageBuilderSharedComponentReferenceMatcher
{
    ...
    public static void ValidateComponentContent(string componentContent)
    {
        ...
        {
            throw new InvalidDataException("Shared Component content is required.");
        }
        ...
        {
            throw new InvalidDataException("Shared Component content must be valid JSON.", ex);
        }
            ...
            rootObject["content"] is not JsonArray content)
        {
            throw new InvalidDataException(
                "Shared Component content must use the TemplateModel shape: { settings: {}, content: [] }.");
        }

        if (ContainsReferenceMarker(rootObject))
        {
            throw new InvalidDataException("Nested Shared Component references are not supported.");
        }
    ...
    internal static bool TryGetReference(
        ...
        out string sharedComponentId)
    {
        var hasExactShape = value.Count == 3 &&
            value.ContainsKey("id") &&
            value.ContainsKey("type") &&
            value.ContainsKey(SharedComponents.ReferenceProperty);
        placementId = GetString(value["id"]);
        sharedComponentId = GetString(value[SharedComponents.ReferenceProperty]);
        var type = GetString(value["type"]);

        return hasExactShape &&
            string.Equals(type, SharedComponents.ReferenceType, StringComparison.Ordinal) &&
            !string.IsNullOrWhiteSpace(placementId) &&
            !string.IsNullOrWhiteSpace(sharedComponentId);
    }
    ...
    private static bool ContainsReferenceMarker(JsonNode node)
    {
        if (node is JsonObject objectNode)
        {
            ...
            if (string.Equals(type, SharedComponents.ReferenceType, StringComparison.Ordinal))
            {
                return true;
            }
    ...
    {
        if (TryGetReference(item, out var placementId, out var sharedComponentId))
        {
            AddReference(references, placementIds, topLevelIds, placementId, sharedComponentId);
            return;
        }
        ...
        string sharedComponentId)
    {
        ...
        {
            throw new InvalidDataException(
                $"Shared Component placement id '{placementId}' conflicts with another top-level content item id.");
        }

        references.Add(sharedComponentId);
    }
        ...
        ISet<string> topLevelIds)
    {
        ...
        {
            throw new InvalidDataException(
                $"Shared Component placement id '{itemId}' conflicts with another top-level content item id.");
        }
    ...
    private static void ValidateNoReferenceMarker(JsonObject item)
    {
        ...
        {
            throw new InvalidDataException(
                "Shared Component references must be complete top-level content items with id, type 'componentRef', and componentRef.");
        }
    }
    ...
    private static void ValidateSections(JsonArray sections, string path)
    {
        ...
        for (var index = 0; index < sections.Count; index++)
        {
            ...
            {
                throw new InvalidDataException($"Shared Component '{itemPath}' must be an object.");
            }
            ...
            {
                throw new InvalidDataException(
                    $"Shared Component '{itemPath}' must have non-empty string id and type properties.");
            }

            if (!siblingIds.Add(id))
            {
                throw new InvalidDataException(
                    $"Shared Component '{path}' contains duplicate sibling id '{id}'.");
            }
            ...
            if (section.ContainsKey("blocks"))
            {
                ...
                {
                    throw new InvalidDataException($"Shared Component '{itemPath}.blocks' must be an array.");
                }
