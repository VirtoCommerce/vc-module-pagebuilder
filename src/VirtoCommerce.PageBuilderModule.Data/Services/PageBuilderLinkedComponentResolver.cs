...
public class PageBuilderLinkedComponentResolver(
    IPageBuilderLinkedComponentContentService contentService,
    ILogger<PageBuilderLinkedComponentResolver> logger)
    : IPageBuilderLinkedComponentResolver
{
    public async Task<string> ResolveAsync(string content, CancellationToken cancellationToken = default)
    {
        ...
        var pageContent = PageBuilderLinkedComponentReferenceMatcher.GetPageContentArray(root);
        ...
        IReadOnlyDictionary<string, string> componentContents)
    {
        if (item is JsonObject itemObject &&
            PageBuilderLinkedComponentReferenceMatcher.TryGetReference(
                itemObject,
                out var placementId,
                out var linkedComponentId))
        {
            AddLinkedComponentSections(
                resolvedContent,
                componentContents,
                placementId,
                linkedComponentId);
    ...
    private void AddLinkedComponentSections(
        JsonArray resolvedContent,
        IReadOnlyDictionary<string, string> componentContents,
        string placementId,
        string linkedComponentId)
    {
        if (!componentContents.TryGetValue(linkedComponentId, out var componentContent) ||
            !TryGetComponentSections(componentContent, out var componentSections))
        {
            logger.LogWarning(
                "Linked Component '{LinkedComponentId}' referenced by placement '{PlacementId}' has no valid content.",
                linkedComponentId,
                placementId);
            return;
        }
