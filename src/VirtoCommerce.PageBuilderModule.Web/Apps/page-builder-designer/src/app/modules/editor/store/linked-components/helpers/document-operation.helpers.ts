import { areSectionsContiguous, isLinkedComponentReference } from '@editor/helpers';
...
export function isSameDocument(
  ...
  return origin.templateKey === templateKey && origin.linkedComponentId === linkedComponentId;
}
...
export function getSelectedSections(template: TemplateModel, selectedIds: string[]): SelectedSectionsResult {
  ...
  if (sections.length !== selectedIds.length || sections.some(isLinkedComponentReference)) {
    return {
      sections: [],
...
export function hasLinkedPlacement(template: TemplateModel, sectionId: string, componentId: string): boolean {
  const section = template.content.find((item) => item.id === sectionId);
  return isLinkedComponentReference(section) && section.componentRef === componentId;
}
