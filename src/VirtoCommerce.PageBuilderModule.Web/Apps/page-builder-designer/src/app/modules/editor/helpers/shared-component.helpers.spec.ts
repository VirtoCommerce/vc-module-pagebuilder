...
import {
    ...
    createSharedComponentReference,
...
describe('shared component helpers', () => {
    it('opens only usage pages that have an identity and are not archived', () => {
        expect(canOpenSharedComponentUsagePage({ id: 'page-1', status: 'Draft' })).toBe(true);
        expect(canOpenSharedComponentUsagePage({ id: 'page-1', status: 'Draft, Published' })).toBe(true);
        expect(canOpenSharedComponentUsagePage({ id: 'page-1', status: 'Archived' })).toBe(false);
        expect(canOpenSharedComponentUsagePage({ id: '  ', status: 'Draft' })).toBe(false);
        expect(canOpenSharedComponentUsagePage({ id: null, status: 'Draft' })).toBe(false);
    });

    it('creates the strict raw marker contract', () => {
        const marker = createSharedComponentReference('component-1', 'placement-1');
        ...
        expect(isSharedComponentReference(marker)).toBe(true);
    });

    it('rejects a marker with any additional field', () => {
        expect(isSharedComponentReference({
            id: 'placement-1',
            type: 'componentRef',
            componentRef: 'component-1',
            name: 'not allowed',
    ...
    it('keeps the raw marker while resolving preview content with stable instance ids', () => {
        ...
        const first = resolveSharedComponents(raw, { 'component-1': shared });
        const second = resolveSharedComponents(raw, { 'component-1': shared });
    ...
    it('resolves the preview shape without adding blocks or provenance', () => {
        const raw = createTemplate({
            content: [createSharedComponentReference('component-1', 'placement-1')],
        });
        const shared = createTemplate({
            content: [{ id: 'shared-section', type: 'hero', name: 'Shared hero' } as unknown as SectionModel],
        });

        const result = resolveSharedComponents(raw, { 'component-1': shared });
        ...
        expect(Object.keys(result.template.content[0]).some(key => key.startsWith('__sharedComponent'))).toBe(false);
    });

    it('keeps placements distinct when their punctuation would sanitize to the same value', () => {
        const raw = createTemplate({
            content: [
                createSharedComponentReference('component-1', 'hero-a'),
                createSharedComponentReference('component-1', 'heroa'),
            ],
        });
        const shared = createTemplate({ content: [createSection({ id: 'shared' })] });
    ...
    it('uses lowercase UTF-8 hex placement tokens across ASCII and Unicode golden vectors', () => {
        ...
        goldenVectors.forEach(([placementId, expectedId]) => {
            const raw = createTemplate({
                content: [createSharedComponentReference('component-1', placementId)],
            });
    ...
    it('omits unresolved references and reports each missing component once', () => {
        const raw = createTemplate({
            content: [
                createSharedComponentReference('missing', 'placement-1'),
                createSharedComponentReference('missing', 'placement-2'),
            ],
        });

        const result = resolveSharedComponents(raw, {});

        expect(result.template.content).toEqual([]);
        expect(result.missingComponentIds).toEqual(['missing']);
    });

    it('inserts shared references without embedding component content', () => {
        const raw = createTemplate({ content: [createSection({ id: 'before' })] });

        const result = insertSharedComponentReference(raw, 'component-1', 0, 'placement-1');
    ...
    it('creates independent copies with regenerated section and nested block ids', () => {
        ...
        const shared = createTemplate({ content: [sourceSection] });
        const raw = createTemplate();

        const result = insertSharedComponentCopy(raw, shared, 0);
    ...
    it('replaces selected sections with one reference at the first selected position', () => {
        ...
        const result = replaceSectionsWithSharedComponent(
            raw,
    ...
    it('rejects non-adjacent sections instead of reordering page content', () => {
        ...
        expect(replaceSectionsWithSharedComponent(raw, ['first', 'last'], 'component-1')).toBe(raw);
    ...
    it('detaches a reference into fresh independent sections', () => {
        ...
        const result = detachSharedComponent(raw, 'placement-1', shared);

        expect(result.content).toHaveLength(1);
        expect(isSharedComponentReference(result.content[0])).toBe(false);
