...
describe('Shared Component document operation helpers', () => {
  ...
  it('accepts only adjacent independent sections', () => {
    ...
    const reference = createSharedComponentReference('component-1', 'reference');
  ...
  it('matches a placement only when both placement and component ids agree', () => {
    ...
    expect(hasSharedComponentPlacement(template, 'placement-1', 'component-1')).toBe(true);
    expect(hasSharedComponentPlacement(template, 'placement-1', 'component-2')).toBe(false);
  });
});
