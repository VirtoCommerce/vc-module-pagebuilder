  ...
  >
    <div class="tw-flex tw-items-start tw-justify-between tw-gap-3">
      ...
      <VcButton
        icon="lucide-x"
        text
        :aria-label="$t('SHARED_COMPONENTS.ACTIONS.CLOSE')"
        @click="$emit('close')"
      />
    </div>
    ...
    <div class="tw-mt-5 tw-space-y-3">
      <div class="tw-text-xs tw-font-semibold tw-uppercase tw-text-[color:var(--neutrals-500)]">
        {{ $t("SHARED_COMPONENTS.DETAILS.COMPONENT_DETAILS") }}
      </div>

      <div class="tw-space-y-1 tw-break-words">
        <VcHint>{{ $t("SHARED_COMPONENTS.DETAILS.STORE") }}</VcHint>
        <div>{{ component.storeId }}</div>
      </div>

      <div class="tw-space-y-1 tw-break-words">
        <VcHint>{{ $t("SHARED_COMPONENTS.DETAILS.MODIFIED") }}</VcHint>
      ...
      >
        <VcHint>{{ $t("SHARED_COMPONENTS.DETAILS.MODIFIED_BY") }}</VcHint>
    ...
    <div class="tw-mt-5 tw-min-h-0 tw-flex-1 tw-space-y-3">
      ...
      <VcSkeleton
        ...
        variant="text"
        :rows="3"
        :aria-label="$t('SHARED_COMPONENTS.DETAILS.LOADING_USAGE')"
      />
      <div
      ...
      <VcHint v-else>
        {{
          $t(
            component.usageCount > 0 ? "SHARED_COMPONENTS.DETAILS.NOT_AVAILABLE" : "SHARED_COMPONENTS.DETAILS.NO_USAGE",
          )
        }}
      </VcHint>
    </div>
    ...
    <div class="tw-mt-5 tw-flex tw-flex-col tw-gap-2">
      ...
      >
        {{ $t("SHARED_COMPONENTS.ACTIONS.RENAME") }}
      </VcButton>
...
interface Emits {
  ...
  (event: "open-designer", page: SharedComponent["usagePages"][number]): void;
}
...
const usedOnTranslationKey = computed(() =>
  props.component.usageCount === 1 ? "SHARED_COMPONENTS.DETAILS.USED_ON_ONE" : "SHARED_COMPONENTS.DETAILS.USED_ON_MANY",
);
const deleteBlockedTranslationKey = computed(() =>
  props.component.usageCount === 1
    ? "SHARED_COMPONENTS.DETAILS.DELETE_BLOCKED_ONE"
    : "SHARED_COMPONENTS.DETAILS.DELETE_BLOCKED_MANY",
);
const formattedModifiedDate = computed(() => {
  const value = props.component.modifiedDate;

  if (!value) {
    return t("SHARED_COMPONENTS.DETAILS.NOT_AVAILABLE");
  }

  const date = value instanceof Date ? value : new Date(value);

  return Number.isNaN(date.getTime())
    ? t("SHARED_COMPONENTS.DETAILS.NOT_AVAILABLE")
    : new Intl.DateTimeFormat(locale.value, { dateStyle: "medium", timeStyle: "short" }).format(date);
});

function canOpenUsagePage(page: SharedComponentUsagePage): boolean {
  return props.canOpenDesigner && Boolean(page.id?.trim()) && page.status !== "Archived";
}

function getUsagePageUnavailableReason(page: SharedComponentUsagePage): string | undefined {
  if (canOpenUsagePage(page)) {
    return undefined;
  }

  if (!page.id?.trim()) {
    return t("SHARED_COMPONENTS.DETAILS.OPEN_UNAVAILABLE_IDENTITY");
  }

  if (page.status === "Archived") {
    return t("SHARED_COMPONENTS.DETAILS.OPEN_UNAVAILABLE_ARCHIVED");
  }

  return t("SHARED_COMPONENTS.DETAILS.OPEN_UNAVAILABLE_PERMISSION");
}
</script>
